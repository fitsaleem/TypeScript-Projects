"use strict";
//  Question Number Guessing Game:
Object.defineProperty(exports, "__esModule", { value: true });
// This guess the number game is a short TypeScript/Node.js project that allows the user to guess the number generated by the computer. There are also several ways to alter the game, like adding more rounds or displaying the score. It’s quite simple and uses the random function to generate a number.
var prompt = require("prompt-sync");
var target;
var score = 0;
var rounds = 10;
var attempts = 0;
var min = 1;
var max = 100;
var history = [];
function getRandomNumber() {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}
function startGame() {
    var ask = prompt({ sigint: true });
    console.log("Welcome to the Guess the Number Game!");
    console.log("I'm thinking of a number between ".concat(min, " and ").concat(max, "."));
    console.log("Can you guess it?");
    target = getRandomNumber();
    askQuestion(ask);
}
function askQuestion(ask) {
    if (rounds === 0) {
        console.log("Game Over! Your score is: ".concat(score));
        console.log('Your successful attempts: ', history);
        var answer_1 = ask('Do you want to play again? (y/n) ');
        if (answer_1.toLowerCase() === 'y') {
            score = 0;
            rounds = 10;
            attempts = 0;
            history = [];
            startGame();
        }
        return;
    }
    var answer = ask('Enter a number: ');
    var number = parseInt(answer);
    if (number === target) {
        console.log('Congratulations, you guessed the number!');
        history.push({ target: target, attempts: attempts });
        score++;
        target = getRandomNumber();
        attempts = 0;
    }
    else {
        attempts++;
        if (attempts % 3 === 0) {
            console.log("Hint: The target number is ".concat(number < target ? 'greater' : 'less', " than your guess."));
        }
        else {
            console.log('Incorrect, try again!');
        }
    }
    rounds--;
    console.log("You have ".concat(rounds, " rounds left."));
    askQuestion(ask);
}
startGame();
// wiht OOP
// import * as readline from 'readline';
// interface GameHistory {
//   target: number;
//   attempts: number;
// }
// class GuessGame {
//   private target: number;
//   private score: number;
//   private rounds: number;
//   private attempts: number;
//   private max: number;
//   private min: number;
//   private history: GameHistory[];
//   constructor(min = 1, max = 100, rounds = 10) {
//     this.min = min;
//     this.max = max;
//     this.target = this.getRandomNumber();
//     this.score = 0;
//     this.rounds = rounds;
//     this.attempts = 0;
//     this.history = [];
//   }
//   private getRandomNumber(): number {
//     return Math.floor(Math.random() * (this.max - this.min + 1)) + this.min;
//   }
//   public startGame(): void {
//     const rl = readline.createInterface({
//       input: process.stdin,
//       output: process.stdout
//     });
//     console.log(`Welcome to the Guess the Number Game!`);
//     console.log(`I'm thinking of a number between ${this.min} and ${this.max}.`);
//     console.log(`Can you guess it?`);
//     this.askQuestion(rl);
//   }
//   private askQuestion(rl: readline.Interface): void {
//     if (this.rounds === 0) {
//       console.log(`Game Over! Your score is: ${this.score}`);
//       console.log('Your successful attempts: ', this.history);
//       rl.question('Do you want to play again? (y/n) ', (answer) => {
//         if (answer.toLowerCase() === 'y') {
//           this.score = 0;
//           this.rounds = 10;
//           this.target = this.getRandomNumber();
//           this.attempts = 0;
//           this.startGame();
//         } else {
//           rl.close();
//         }
//       });
//       return;
//     }
//     rl.question('Enter a number: ', (answer) => {
//       const number = parseInt(answer);
//       if (number === this.target) {
//         console.log('Congratulations, you guessed the number!');
//         this.history.push({ target: this.target, attempts: this.attempts });
//         this.score++;
//         this.target = this.getRandomNumber();
//         this.attempts = 0;
//       } else {
//         this.attempts++;
//         if (this.attempts % 3 === 0) {
//           console.log(`Hint: The target number is ${number < this.target ? 'greater' : 'less'} than your guess.`);
//         } else {
//           console.log('Incorrect, try again!');
//         }
//       }
//       this.rounds--;
//       console.log(`You have ${this.rounds} rounds left.`);
//       this.askQuestion(rl);
//     });
//   }
// }
// const game = new GuessGame(1, 100, 10);
// game.startGame();
